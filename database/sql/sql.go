...
func Shrink(d time.Duration, c context.Context) (time.Duration, context.Context, context.CancelFunc) {
	if deadline, ok := c.Deadline(); ok {
		if ctimeout := time.Until(deadline); ctimeout < time.Duration(d) {
			// deliver small timeout
			return time.Duration(ctimeout), c, func() {}
		}
	}
...
func (db *DB) QueryRow(c context.Context, query string, args ...interface{}) *Row {
	...
	for i := range db.read {
		...
		if row.err == nil || row.err == ErrNoRows {
			return row
		}
	}
...
func (db *conn) begin(c context.Context) (tx *Tx, err error) {
	_, c, cancel := Shrink(db.conf.TranTimeout, c)
...
func (db *conn) exec(c context.Context, query string, args ...interface{}) (res sql.Result, err error) {
	_, c, cancel := Shrink(db.conf.ExecTimeout, c)
...
func (db *conn) query(c context.Context, query string, args ...interface{}) (rows *Rows, err error) {
	_, c, cancel := Shrink(db.conf.QueryTimeout, c)
...
func (s *Stmt) Exec(c context.Context, args ...interface{}) (res sql.Result, err error) {
	...
	_, c, cancel := Shrink(s.db.conf.ExecTimeout, c)
...
func (s *Stmt) Query(c context.Context, args ...interface{}) (rows *Rows, err error) {
	...
	_, c, cancel := Shrink(s.db.conf.QueryTimeout, c)
